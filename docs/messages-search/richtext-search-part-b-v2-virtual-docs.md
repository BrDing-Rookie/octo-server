...
> 上游 OS：测试环境 OpenSearch 集群（2.17.x，消息索引 + 读别名）。
...
- 实测：测试集群现有少量 type=14 富文本 doc（示例），
...
4. **测试环境验证**：
   ...
   - 前端走测试环境部署触发 `_search_media` / `_search_files` 查富文本里的"合同
...
回滚成本：一次性 reindex 或 delete by query，10 分钟内完成（测试集群 type=14 数据量很小）。
